...
def main():
    ...
    # 定义不同的超参数配置
    configs = [
        #{"num_epochs": 50, "lr": 0.001, "momentum": 0.9, "batch_size": 32, "weight_decay": 1e-4},
        #{"num_epochs": 50, "lr": 0.005, "momentum": 0.9, "batch_size": 64, "weight_decay": 1e-4},
        {"num_epochs": 50, "lr": 0.001, "momentum": 0.95, "batch_size": 32, "weight_decay": 1e-4},
        {"num_epochs": 50, "lr": 0.002, "momentum": 0.95, "batch_size": 32, "weight_decay": 1e-4},
        {"num_epochs": 50, "lr": 0.001, "momentum": 0.94, "batch_size": 32, "weight_decay": 1e-4},
        {"num_epochs": 40, "lr": 0.001, "momentum": 0.95, "batch_size": 32, "weight_decay": 1e-4},
        #{"num_epochs": 50, "lr": 0.001, "momentum": 0.95, "batch_size": 64, "weight_decay": 1e-4},
    ]
    ...
    for i, config in enumerate(configs):
        ...
        save_path = f'./modelset/L2_regularization_model/config_{i+1}'
        if rank == 0:
            os.makedirs(save_path, exist_ok=True)
        ...
        # 保存最终的模型权重
        #if rank == 0:
        #    final_model_path = f'./modelset/L2_regularization_model/config_{i+1}_final_model.pth'
        #    torch.save(net.module.state_dict(), final_model_path)
        #    print(f"Final model for config_{i+1} saved to {final_model_path}")

    if rank == 0:
        # 创建目录
        os.makedirs('./modelset/L2_regularization_model', exist_ok=True)
        
        with open('./modelset/L2_regularization_model/training_results.json', 'w') as f:
